export const STORE_FETCHED_OPTIONS = 'STORE_FETCHED_OPTIONS'
export const OPTIONS_LOADED = 'OPTIONS_LOADED'

export const UI_MODAL_PDF_URL_CHANGED = 'UI_MODAL_PDF_URL_CHANGED'
export const UI_MODAL_TOGGLED = 'UI_MODAL_TOGGLED'
export const UI_MENU_TOGGLED = 'UI_MENU_TOGGLED'
export const UI_MENU_VISIBLE = 'UI_MENU_VISIBLE'
export const UI_MOBILE_TOGGLED = 'UI_MOBILE_TOGGLED'

export const STORE_FETCHED_CATEGORIES = 'STORE_FETCHED_CATEGORIES'
export const CATEGORIES_LOADED = 'CATEGORIES_LOADED'

export const STORE_FETCHED_PAGES = 'STORE_FETCHED_PAGES'
export const PAGES_LOADED = 'PAGES_LOADED'

export const STORE_FETCHED_POSTS = 'STORE_FETCHED_POSTS'
export const POSTS_LOADED = 'POSTS_LOADED'

export const STORE_FETCHED_ACTIVITIES = 'STORE_FETCHED_ACTIVITIES'
export const ACTIVITIES_LOADED = 'ACTIVITIES_LOADED'

export const STORE_FETCHED_GRANTS = 'STORE_FETCHED_GRANTS'
export const GRANTS_LOADED = 'GRANTS_LOADED'

export const STORE_FETCHED_EVENTS = 'STORE_FETCHED_EVENTS'
export const EVENTS_LOADED = 'EVENTS_LOADED'

export const LOCALE_CHANGED = 'LOCALE_CHANGED'

export const STORE_FETCHED_USER = 'STORE_FETCHED_USER'
export const LOGIN_USER = 'LOGIN_USER'
export const LOGOUT_USER = 'LOGOUT_USER'

export const INCREMENT_LOADING_PROGRESS = 'INCREMENT_LOADING_PROGRESS'
export const RESET_LOADING_PROGRESS = 'RESET_LOADING_PROGRESS'
